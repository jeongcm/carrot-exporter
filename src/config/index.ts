...
export default {
  ...
  maxApiBodySize: process.env.CM_EXPORTER_MAX_API_BODY_SIZE || '50mb',
  ...
  victoriaMetrics: {
    ...
    CM_EXPORTER_VM_SINGLE_ADDRESS: process.env.CM_EXPORTER_VM_SINGLE_ADDRESS,
    ...
    vmMultiBaseUrlInsert:
      process.env.CM_EXPORTER_VM_MULTI_BASE_URL_INSERT || 'http://localhost',
    vmMultiBaseUrlSelect:
      process.env.CM_EXPORTER_VM_MULTI_BASE_URL_SELECT || 'http://localhost',
    vmMultiAuthUrl: process.env.CM_EXPORTER_VM_MULTI_AUTH_URL || 'http://localhost',
    vmMultiNamespaces: process.env.CM_EXPORTER_VM_MULTI_NAMESPACE || 'vm-multi-tenant',
    vmMultiSecret: process.env.CM_EXPORTER_VM_MULTI_SECRET || 'vm-auth-victoria-metrics-auth',
    vmMultiClusterUuid: process.env.CM_EXPORTER_VM_MULTI_CLUSTER_UUID,
    vmOption: process.env.CM_EXPORTER_VM_OPTION || 'SINGLE',
    vmOpenstackSwitch: process.env.CM_EXPORTER_VM_OPENSTACK_SWITCH || 'off',
  },

  obsUrl: {
    kpsNamespace: process.env.CM_EXPORTER_KPS_NAMESPACE || 'monitor',
    prometheusUrlHead: process.env.CM_EXPORTER_PROMETHEUS_URL_HEAD || 'http://kps-kube-prometheus-stack-prometheus.',
    prometheusUrlTail: process.env.CM_EXPORTER_PROMETHEUS_URL_TAIL || '.svc.cluster.local:9090',
    nodeExporterPort: process.env.NODE_EXPORTER_PORT || 9100,
  },
  oT: {
    oTTraceLogTurnOff: process.env.CM_EXPORTER_TURN_OFF_TELEMETRY,
  },
  ...
  alert: {
    alertExpiredDate: process.env.CM_EXPORTER_ALERT_EXPIRED_DATE || 60 * 60 * 24 // 1d
  },
