export default async function getCloudMongoDBInstanceMetric(totalMsg, clusterUuid) {
  // initialize result
  let queryResult = {
  ...
  totalMsg.result[0][0]?.forEach((cloudMongoDBInstance) => {
    cloudMongoDBInstance?.outputs?.forEach((output) => {
