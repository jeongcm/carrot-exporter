...
export default async function getServerImageProductListQuery(result, clusterUuid) {
  ...
  const resourceType = 'SIP';
  const resultLength = result.getMemberServerImageInstanceListResponse?.memberServerImageInstanceList?.length;
  for (let i = 0; i < resultLength; i++) {
    ...
    query['resource_Target_Created_At'] = result.getMemberServerImageInstanceListResponse?.memberServerImageInstanceList[i]?.createDate;
    // query['resource_Namespace'] =
    // query['parent_Resource_Id'] =
    query['resource_Status'] = result.getMemberServerImageInstanceListResponse?.memberServerImageInstanceList[i]?.memberServerImageInstanceStatusName;
