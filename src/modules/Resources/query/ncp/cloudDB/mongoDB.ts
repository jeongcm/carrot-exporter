...
export default async function getCloudDBMongoDbInstanceListQuery(result, clusterUuid) {
  let query = {};
  let mergedQuery = {};
  let tempQuery = {};
  ...
  for (let i = 0; i < resultLength; i ++) {
    let instanceLength = result[i][0].outputs.getCloudMongoDbInstanceDetailResponse.cloudMongoDbInstanceList.length
